...
#' Download stock quote from Rediff Money
#'
#' @param stock_code Stock code.
#'
#' @noRd
#'
nse_stock_quote_rediff <- function(stock_code) {
  ...
  colnames(result) <- c("symbol", "isin")

  isin      <- result[result$symbol == toupper(stock_code), ]$isin
  base_url  <- "https://money.rediff.com/companies/"
  stock_url <- paste0(base_url, isin)
  read_url  <- xml2::read_html(stock_url)

  read_url %>%
    rvest::html_nodes(xpath = '//span[@id="ltpid_nse"]') %>%
    rvest::html_text() %>%
    as.numeric()
